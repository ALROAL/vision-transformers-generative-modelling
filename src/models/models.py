...
class ViTCVAE(LightningModule):
    def __init__(
        ...
        self.decoder_pos_embedding = nn.Parameter(torch.randn(1, num_patches + 3, dim))
    ...
    def forward(self, img, label):
        if label.shape[1] == 40:
            label = label[:,20].to(torch.float)
        else:
            label = label.to(torch.float)
        
        x = self.encoder(img,label)
